...
## Prečo potrebujeme vetvenie?

- bez vetvenia vývojári upravujú živý kód → nedokončené zmeny rozbíjajú systém
- vývojári potrebujú **izoláciu**, aby mohli pracovať bez vzájomného zasahovania
- zároveň je potrebné hotové zmeny **integrovať** späť do hlavnej línie vývoja
...
## Prečo potrebujeme workflowy vetvenia?

- vetvenie poskytuje ilúziu zastaveného času - bezpečnú izoláciu pre vývoj
- ale: *vetvenie je jednoduché, zlučovanie je ťažšie* - v istom momente treba "splatiť dlh"
- **kľúčová otázka:** ako a kedy by sme mali integrovať našu prácu?
- neexistuje univerzálne riešenie: rôzne tímy, produkty a kontexty vyžadujú rôzne prístupy k vetveniu.
...
- môže odrádzať od refaktoringu kvôli zvýšeným merge konfliktom

:::footer
[Git feature branch workflow](https://www.atlassian.com/git/tutorials/comparing-workflows/feature-branch-workflow) (Atlassian)
:::

## {background-image="https://www.tilburgsciencehub.com/static/img/feature-branching.webp" background-size="80%" background-position="center"}
...
- feature vetvy sa niekedy nepoužívajú vôbec - vývojári commitujú priamo do mainu
...
- vyžaduje zrelý vývojový tím a dôveru
...
## Menšie tímy {background-image="https://trunkbaseddevelopment.com/trunk1b.png" background-size="60%" background-position="center"}
...
## Väčšie tímy {background-image="https://trunkbaseddevelopment.com/trunk1c.png" background-size="60%" background-position="center"}
...
- potenciálne dobrý fit pre projekty s viacerými produkčnými verziami
...
1. **Vetvenie je jednoduché, zlučovanie je ťažšie** - vždy naplánujte integráciu pred vytvorením vetvy; vetvy sa v čase rozchádzajú

2. **Frekvencia integrácie súvisí s kvalitou** - časté zlučovanie snižuje zložitosť, riziko a strach z integrácie; umožňuje refaktoring a zlepšuje zdravie kódu

3. **Moderná osvedčená prax uprednostňuje trunk-based development** - elitné tímy integrujú často s CI/CD
  
4. **Voľba workflowu závisí od kontextu** - rôzne tímy a projekty majú rôzne potreby; vyberte workflow, ktorý najlepšie vyhovuje vašej situácii

:::footer
Podľa [Patterns for Managing Source Code Branches](https://martinfowler.com/articles/branching-patterns.html) (Martin Fowler) a [State of DevOps Report](https://martinfowler.com/bliki/StateOfDevOpsReport.html)
:::

<!-- ## {background-image="./static/img/branching-feature.webp" background-size="auto 70%" background-position="center"}

## {background-image="./static/img/branching-forking-workflow.webp" background-size="contain" background-position="center"}

## {background-image="./static/img/branching-gitflow.webp" background-size="contain" background-position="center"}

## {background-image="./static/img/branching-github-flow.webp" background-size="contain" background-position="center"}

## {background-image="./static/img/branching-gitlab-flow.webp" background-size="contain" background-position="center"}

## {background-image="./static/img/branching-release-workflow.webp" background-size="contain" background-position="center"}

## {background-image="./static/img/branching-trunk-based-development.webp" background-size="contain" background-position="center"}

## {background-image="./static/img/branching-env.webp" background-size="contain" background-position="center"} -->
